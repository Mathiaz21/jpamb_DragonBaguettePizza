...
class Master:
  ...
  __slave_should_print: bool = False

  def __init__(self, method_id: str, should_print:bool) -> None:
    
    self.__method = MethodId.parse(method_id)
    self.__slave_should_print = should_print
  ...
  def initialize_slave_list(self):
    if len(self.__method.params) == 0:
      self.initialize_single_slave()
    else: 
      self.initialize_several_slaves()



  def initialize_single_slave(self):
      self.__slave_list.append(Slave(self.__program_bytecode_file_path, self.__method_name, self.__reports_from_slaves, should_print_process=self.__slave_should_print))


  def initialize_several_slaves(self):
    ...
    for param_1_value in possible_params_values[0]:
      ...
      if possible_params_values[1]:
        for param_2_value in possible_params_values[1]:
          ...
          self.__slave_list.append( Slave(self.__program_bytecode_file_path, self.__method_name, self.__reports_from_slaves,params, should_print_process=self.__slave_should_print) )
      else:
